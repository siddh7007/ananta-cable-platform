...
export function attachRequestLogging(fastify: FastifyInstance): void {
  ...
  fastify.addHook('onSend', (req, reply, payload, done) => {
    if (typeof payload === 'string') {
      (req as RequestWithTiming).resSize = Buffer.byteLength(payload, 'utf8');
    } else if (Buffer.isBuffer(payload)) {
      (req as RequestWithTiming).resSize = payload.length;
    }
    done();
  });
  ...
  fastify.addHook('onResponse', (req, reply, done) => {
    ...
    const startTime = (req as RequestWithTiming).startTime as bigint;
    ...
    const resSize = (req as RequestWithTiming).resSize || 0;
