...
def run_drc(request: Union[DRCRunRequest, AssemblySchema]):
    ...
    try:
        if isinstance(request, AssemblySchema):
            ...
            assembly = drc_engine.load(request.assembly_id)
            if assembly is None:
...
def apply_fixes(request: DRCApplyFixesRequest):
    ...
    try:
        schema_hash, report = drc_engine.apply_fixes(assembly, request.fix_ids, request.ruleset_id)
        return DRCApplyFixesResponse(assembly_id=request.assembly_id, schema_hash=schema_hash, drc=report)
    except Exception as exc:
