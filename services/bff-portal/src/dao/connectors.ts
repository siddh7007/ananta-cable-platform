...
import type { ConnectorSummary, ConnectorMetadata } from '@cable-platform/contracts/types/api';

export class ConnectorsDAO {
  /**
   * Search connectors with fuzzy matching
   */
  async searchConnectors(query: string, limit: number = 20): Promise<ConnectorSummary[]> {
   ...
   */
  async getConnectorMetadata(mpn: string): Promise<ConnectorMetadata | null> {
    const result = await sql`
