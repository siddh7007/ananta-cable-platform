...
import type { AssemblyDraft, AssemblyDraftResponse, AssemblyStep1 } from '@cable-platform/contracts/types/api';

export interface DraftWithPayload extends AssemblyDraft {
  draft_id: string;
  user_id: string;
  created_at: string;
  updated_at: string;
...
export class AssembliesDAO {
   ...
   */
  async saveDraft(userId: string, draft: AssemblyDraft): Promise<AssemblyDraftResponse> {
   ...
   * Get a draft by ID
   */
  async getDraft(draftId: string): Promise<DraftWithPayload | null> {
    const result = await sql`
    ...
    return {
      ...
      user_id: row.user_id,
      step: row.step,
      payload: row.payload as AssemblyStep1,
      status: row.status,
   ...
   */
  async getUserDrafts(userId: string): Promise<DraftWithPayload[]> {
    const result = await sql`
      SELECT draft_id, user_id, step, payload, status, created_at, updated_at
      FROM assemblies_draft
