...
export async function synthesisRoutes(fastify) {
    ...
    fastify.post('/v1/synthesis/recompute', {
        preHandler: requireAuth,
        handler: async (request, reply) => {
            const { draft_id, locks } = request.body;
            const userId = request.user.sub;
            try {
    ...
    fastify.post('/v1/synthesis/accept', {
        preHandler: requireAuth,
        handler: async (request, reply) => {
            const { proposal_id, locks } = request.body;
            try {
                // 1. Load proposal
                ...
                const schemaHash = crypto.createHash('sha256')
                    ...
                    .digest('hex');
                const schemaRecord = await synthesisDao.saveSchema(proposalRecord.draft_id, schema, schemaHash);
                // 4. Return result
